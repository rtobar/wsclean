...
public:
	...
	void MakeImage(const ImageFilename& imageName, const ImagingTableEntry& entry, size_t joinedChannelIndex, const ImageWeightCache* imageWeightCache, ImageBufferAllocator& allocator)
	{
		...
		if(_settings.polarizations.size() == 1)
		{
			...
			if(pol == Polarization::StokesI)
			{
				...
				FitsReader reader(stokesIName.GetPrefix(_settings) + "-image.fits");
				...
				writer.Write(stokesIName.GetPrefix(_settings) + "-image-pb.fits", image.data());
			}
			else {
		...
		else if(_settings.polarizations.size() == 4)
		{
			if(Polarization::HasFullStokesPolarization(_settings.polarizations))
			{
				...
				for(size_t polIndex = 0; polIndex != 4; ++polIndex)
				{
					...
					writer.Write(name.GetPrefix(_settings) + "-image-pb.fits", images[polIndex].data());
				}
			}
		}
		else {
...
private:
	...
	void applyStokesI(ImageBufferAllocator::Ptr& stokesI)
	{
		...
		for(size_t j=0; j!=size; ++j)
		{
			...
			if(val.Invert())
				stokesI[j] = stokesI[j] * 0.5 * (val[0].real() + val[3].real());
			else
				stokesI[j] = std::numeric_limits<double>::quiet_NaN();
		}
	}
	
	void applyFullStokes(ImageBufferAllocator::Ptr images[4])
	{
		size_t size = _settings.trimmedImageWidth * _settings.trimmedImageHeight;
		for(size_t j=0; j!=size; ++j)
		{
